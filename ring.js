...
const rows = db.prepare('SELECT id, seen_count, sessions, app_seeder FROM nodes WHERE id IS NOT NULL').all()
...
<head>
  <meta charset="utf-8" />
  <title>dht-explorer · keyspace ring</title>
...
<body>
  <div id="title" class="panel">
    <h1>dht-explorer · <span class="accent">keyspace ring</span></h1>
...
fs.writeFileSync('ring.html', html)
const cwd = (process.cwd && process.cwd()) || '.'
console.log(`ring: wrote ring.html`)
console.log(`open it in a browser:  file://${cwd}/ring.html`)
db.close()
process.exit(0)
