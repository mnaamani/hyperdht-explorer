...
export async function run(ctx) {
  ...
  for (let i = 0; i < rest.length; i++) {
    const a = rest[i]
    if (a === '--seed') SEED = true
    else if (a === '--minutes') MINUTES = Number(rest[++i])
...
`)
  ...
  function recordConn(conn) {
    ...
    if (conn.rawStream && conn.rawStream.remoteHost) read()
    else conn.once('open', read)
  }
  ...
  async function runSeed() {
    ...
    const tally = (acc) => (index, byteLength, from) => {
      ...
      if (from && from.remotePublicKey) acc.peers.add(b4a.toString(from.remotePublicKey, 'hex'))
    }
    const attach = (core) => {
    ...
    function logServed(tick) {
      ...
      const repl = Math.max(drive.core.peers.length, blobsCore ? blobsCore.peers.length : 0)
      console.log(
    ...
    const servedTimer = globalThis.setInterval(() => logServed(true), 60 * 1000)
