...
// timeline page.
//
//   npx bare storeprobe.js [--canaries N] [--checkpoints 0,5,10,15,20,22]
//
...
const timeout = (ms) => new Promise((_, rej) => globalThis.setTimeout(() => rej(new Error('timeout')), ms))
...
for (let i = 0; i < CANARIES; i++) {
  ...
  try {
    ...
    console.log(`  put ${b4a.toString(hash, 'hex').slice(0, 16)}… on ${closestNodes.length} node(s)`)
  } catch (err) {
