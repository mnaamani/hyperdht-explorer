...
// ---------------------------------------------------------------------------
// dht-explorer: random-walk crawler for the hyperdht (Kademlia) node network.
//
...
//
// Modes:
//   npx bare index.js                 random-walk crawl (runs until stopped)
//   npx bare index.js --for 60        crawl for ~60 seconds, then stop cleanly
//   npx bare index.js --queries 50    crawl until 50 findNode queries, then stop
//   npx bare index.js --prune-hours N drop nodes not seen in N hours (default 72; 0 disables)
//   npx bare index.js <topic-hex>     lookup announcers for a specific topic hash
//
...
// NOT deliver SIGINT/SIGTERM to JS handlers, so `timeout npx bare ...` would just
...
const stmtExists = db.prepare('SELECT first_seen, sessions FROM nodes WHERE host = ? AND port = ?')
...
const stmtBump = db.prepare(`
  ...
  WHERE host = ? AND port = ?
`)
const stmtKnownPeers = db.prepare('SELECT host, port FROM nodes ORDER BY last_seen DESC, sessions DESC LIMIT ?')
...
let prunedThisRun = 0 // stale nodes removed during this run
...
function record(node) {
  ...
  if (!prior) {
    ...
    console.log(`~ known  ${addr.padEnd(21)} sessions=${prior.sessions + 1}, first seen ${ago(prior.first_seen)} ago`)
  }
}
...
function writeSnapshot() {
  ...
  const seeders = db.prepare('SELECT COUNT(*) AS n FROM nodes WHERE app_seeder IS NOT NULL').get().n
  ...
  db.prepare(
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `
  ).run(Date.now(), total, alive, newThisRun, prunedThisRun, countries.size, asns.size, seeders, medianRtt, observed)
  console.log(
