...
const db = openDb()
const stmtAlive = db.prepare('UPDATE nodes SET alive = 1, rtt_ms = ?, last_ping = ? WHERE host = ? AND port = ?')
const stmtDead = db.prepare('UPDATE nodes SET alive = 0, rtt_ms = NULL, last_ping = ? WHERE host = ? AND port = ?')

const targets = db.prepare('SELECT host, port FROM nodes ORDER BY last_seen DESC').all()
console.log(`probe: pinging ${targets.length} node(s) (timeout ${PING_TIMEOUT}ms, ${CONCURRENCY} concurrent)\n`)
...
async function probe(node) {
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) => globalThis.setTimeout(() => reject(new Error('timeout')), PING_TIMEOUT))
    ])
  ...
  if (++done % 50 === 0) console.log(`  ${done}/${targets.length}  (${alive} alive, ${dead} dead)`)
}
