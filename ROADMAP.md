# Roadmap: long-running daemon, seeding at scale, federation

Planned work — everything here is **proposed / not started**. Shipped behaviour is
documented in the README and CLAUDE.md, not here.
...
hyperdht-explorer crawls from a single vantage point and stores everything in one
...
Two directions, both enabled by the same shift from a one-shot CLI to a
**long-running daemon**:

1. **Seeding at scale** — `observe` already replicates and serves the public
   app-update feed it watches, but only for the length of one bounded run and with
   no resource budget. A daemon turns that into sustained, governed seeding of
   several apps.
2. **Federation** — pool observations across many explorers.

The daemon section below is the foundation; both build on it.

## From one-shot CLI to a long-running daemon

Every command today is a one-shot process: run, do work, exit. Sustained seeding and
federation need the opposite — a node that **stays online**, holds swarm membership,
and replicates over time. So the enabling shift is a long-running **daemon** mode.
...
## Seeding at scale — what a daemon still has to solve

The replication mechanics are done: `observe` resolves `pear://` → the app's update
drive, joins its discovery key as server + client, replicates into a corestore, and
serves blocks, recording connecting peers as aggregate health along the way. What is
missing is everything that makes it safe to leave running.

**The bright line (non-negotiable, and unchanged):** seed **only public app-update
feeds** — the `pear://` link's Hyperdrive, which is _designed_ to be reseeded by every
install. **Never** private/room data (Keet chat rooms, per-room keys); those aren't
discoverable and are off-limits (see the health-not-deanon intent in CLAUDE.md).

The open work is resources and policy, not transport:

- **Resource governance.** Seeding popular apps continuously means real disk + upload
  bandwidth. Needs an explicit **allowlist** (opt-in per app — never auto-seed
  everything we observe), per-app + global disk/bandwidth caps, and an eviction
  policy. This is the bulk of the work.
- **Redistribution.** We re-serve third-party software feeds. For open P2P update
  drives that is the intended model (every install reseeds), but keep it opt-in and
  don't seed anything whose distribution looks restricted.
- **Privacy posture under a stable identity.** The seeder keypair is persisted, so a
  daemon has a steady, long-lived footprint rather than a per-run one. The
  health-only/aggregate rule still governs — counts + `/24` geo, never per-identity
  logs — but a permanently-online seeder sees far more replication peers than a
  20-minute run, so that rule needs enforcing rather than assuming.
...
- **Identity vs. health-only.** Does an always-on seeder identity weaken the
...
Two tracks; the daemon substrate (D0) underpins both. The seeding track is the
nearer-term, higher-value-to-the-network work; federation (F-track) is the larger
follow-on.

**Daemon + governed seeding**

- **D0 — Daemon skeleton.** Long-running mode + worker backend + CLI-as-IPC-client;
  move `observe` into it. No persistent seeding yet — just prove the daemon/IPC shape.
- **D1 — Caps + allowlist.** `seed add <pear://>` puts one app under sustained
  seeding with hard disk/bandwidth caps and explicit public-feed-only enforcement.
...
- `seeders.mjs` already demonstrates the announce/lookup rendezvous against a real
  topic and the `pear://` → discovery-key resolution seeding needs — reuse both.
- `observe.mjs` already joins an app's discovery key as server + client and
  replicates its drive into a corestore, bounded by `--minutes`. The daemon hosts
  that same swarm without the bound, and adds the budgets around it.
