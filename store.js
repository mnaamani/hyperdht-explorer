...
//     npx bare store.js put  "<text>"        -> prints the hash
//     npx bare store.js get  <hash-hex>      -> fetches by hash
//
//   mutable (key-addressed, signed, updatable):
//     npx bare store.js mput "<text>" [seed-hex] [seq]  -> prints public key
//     npx bare store.js mget <publicKey-hex>            -> fetches latest
//
...
try {
  if (mode === 'put') {
    ...
    console.log('\nread back with:  npx bare store.js get', b4a.toString(hash, 'hex'))
  } else if (mode === 'get') {
    if (!a) usage()
    const node = await dht.immutableGet(b4a.from(a, 'hex'))
    if (!node) console.log('not found (may have expired, or never stored)')
    else console.log('value  :', b4a.toString(node.value), '\nfrom   :', node.from.host + ':' + node.from.port)
  } else if (mode === 'mput') {
    if (!a) usage()
    ...
    console.log('\nread back with:  npx bare store.js mget', b4a.toString(keyPair.publicKey, 'hex'))
  } else if (mode === 'mget') {
    if (!a) usage()
